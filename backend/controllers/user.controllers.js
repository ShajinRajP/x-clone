...
export const updateUser = async(req, res)=>{
    try{
        ...
        if((!newPassword && currentPassword)|| (!currentPassword && newPassword)){
            return res.status(400).json({error: "Please provide New Password and current Password"})
        }
        ...
        user.password = null; // to not share the actual password to the frontend
        
        return res.status(200).json(user);

    }catch(error){
        console.log(`Error in UserUpdate controller ${error}`)
        res.status(500).json({error:"Internal Server Error"})
    }
}